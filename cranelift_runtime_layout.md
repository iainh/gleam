...
- **Integers**: `(value << 1) | 0b1`, where `value` is a signed 63-bit integer. Overflow checks in codegen bail out to runtime helpers.
- **Booleans**: `0b001` for `False`, `0b011` for `True`.
- **Nil / empty list**: `0b111` denotes the singleton empty list as well as the unit value.
- **Atoms**: `0b101` plus higher bits storing an index into a global atom table (`index << 3 | 0b101`). Atoms beyond the fixnum range are boxed automatically.
...
- The empty list is the immediate `0b111`.
...
- Reference-counted buffers (strings/binaries) use atomic counters to enable future multi-threading.
- All other heap objects participate in a tracing GC (initial implementation can be single-generation mark-sweep). The `size` field in headers makes scanning straightforward.
- Roots come from the stack maps generated by Cranelift; closures, list cells, and tuples are all scanned uniformly.
